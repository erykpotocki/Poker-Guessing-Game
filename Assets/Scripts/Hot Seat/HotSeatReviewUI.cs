...
public sealed class HotSeatReviewUI : MonoBehaviour
{
    ...
    private void Build(RectTransform root,List<Hand> hands,string title,Sprite back)
    {
        ...
        var viewport=Box(root,"PlayersViewport",new Vector2(.035f,.23f),new Vector2(.965f,.70f));
        ...
        float cardWidth=Mathf.Min(125,(width*.76f-48)/6f);
        ...
        for(int row=0;row<hands.Count;row++)
        {
            ...
            for(int c=0;c<hand.Cards.Count;c++)
            {
                ...
                inspect.onClick.AddListener(()=>{if(finished)InspectCard(selected,playerName);});
            }
        }
        verdict=Text(root,"Odkrywam karty kolejno od gracza rozpoczynającego…",new Vector2(.045f,.095f),new Vector2(.955f,.215f),30);
        var button=Box(root,"ReviewAction",new Vector2(.16f,.025f),new Vector2(.84f,.085f));
    ...
    private IEnumerator Reveal()
    {
        ...
        {
            float duration=0;
            while(duration<(fast ? .035f : .38f)){duration+=Time.unscaledDeltaTime;yield return null;}
        ...
        verdict.text=(complete?"<color=#57E878>UKŁAD JEST NA STOLE</color>":"<color=#FF6660>UKŁADU NIE MA NA STOLE</color>")+"\n"+result;
        finished=true;action.interactable=true;actionLabel.text="KONTYNUUJ";
    ...
    private void InspectCard(CardSpriteEntry card,string playerName)
    {
        ...
        close.onClick.AddListener(()=>Destroy(panel.gameObject));
    ...
    private void RefreshEvidence()
    {
        ...
        foreach(Entry entry in entries)
        {
            ...
            if(outline!=null){outline.enabled=selected;outline.effectDistance=new Vector2(3,-3);outline.effectColor=complete?new Color(.2f,1,.4f):new Color(1,.77f,.15f);}
        }
